...
CREATE TABLE IF NOT EXISTS lgu_alert (
  id INT PRIMARY KEY DEFAULT 1, active BOOLEAN DEFAULT FALSE,
  title TEXT DEFAULT '', body TEXT DEFAULT '', updated_at TIMESTAMPTZ DEFAULT NOW()
);
INSERT INTO lgu_alert (id, active, title, body) VALUES (1, FALSE, '', '') ON CONFLICT (id) DO NOTHING;

CREATE TABLE IF NOT EXISTS lgu_announcements (
  id BIGSERIAL PRIMARY KEY, text TEXT NOT NULL, active BOOLEAN DEFAULT TRUE,
  sort_order INT DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_articles (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), title TEXT NOT NULL, tag TEXT DEFAULT 'news',
  summary TEXT, body_html TEXT, cover_url TEXT, date DATE, published BOOLEAN DEFAULT FALSE,
  author TEXT, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_events (
  id BIGSERIAL PRIMARY KEY, title TEXT NOT NULL, date DATE, time TEXT, venue TEXT,
  type TEXT DEFAULT 'Other', description TEXT, active BOOLEAN DEFAULT TRUE,
  sort_order INT DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_gallery (
  id BIGSERIAL PRIMARY KEY, image_url TEXT NOT NULL, caption TEXT,
  sort_order INT DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_hero_slides (
  id BIGSERIAL PRIMARY KEY, image_url TEXT NOT NULL, caption TEXT, subtitle TEXT,
  sort_order INT DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_officials (
  id BIGSERIAL PRIMARY KEY, name TEXT, role TEXT, bio TEXT, party TEXT, term TEXT,
  emoji TEXT DEFAULT '🧑‍💼', photo_url TEXT, sort_order INT DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_services (
  id BIGSERIAL PRIMARY KEY, icon TEXT DEFAULT '📋', title TEXT NOT NULL, description TEXT,
  link TEXT DEFAULT '#', arrow TEXT DEFAULT 'Learn More', sort_order INT DEFAULT 0
);

CREATE TABLE IF NOT EXISTS lgu_documents (
  id BIGSERIAL PRIMARY KEY, title TEXT NOT NULL, category TEXT DEFAULT 'Other',
  type TEXT DEFAULT 'PDF', url TEXT NOT NULL, active BOOLEAN DEFAULT TRUE,
  sort_order INT DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_barangays (
  id BIGSERIAL PRIMARY KEY, name TEXT NOT NULL, emoji TEXT DEFAULT '🏘️',
  captain TEXT, population TEXT, area TEXT, description TEXT, sort_order INT DEFAULT 0
);

CREATE TABLE IF NOT EXISTS lgu_directory (
  id BIGSERIAL PRIMARY KEY, dept TEXT, name TEXT NOT NULL, head TEXT, phone TEXT,
  email TEXT, hours TEXT DEFAULT 'Mon–Fri, 8AM–5PM', sort_order INT DEFAULT 0
);

CREATE TABLE IF NOT EXISTS lgu_faq (
  id BIGSERIAL PRIMARY KEY, question TEXT NOT NULL, answer TEXT NOT NULL, sort_order INT DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS lgu_users (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), username TEXT UNIQUE NOT NULL, name TEXT, dept TEXT,
  role TEXT NOT NULL DEFAULT 'viewer' CHECK (role IN ('super','editor','compliance','viewer')),
  pass_hash TEXT NOT NULL, mfa_enabled BOOLEAN DEFAULT FALSE, mfa_secret TEXT DEFAULT '',
  active BOOLEAN DEFAULT TRUE, force_pass_change BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS lgu_audit (
  id BIGSERIAL PRIMARY KEY, type TEXT NOT NULL, action TEXT NOT NULL, username TEXT,
  user_name TEXT, user_role TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE POLICY "system_write_audit" ON lgu_audit FOR INSERT WITH CHECK (true);
CREATE POLICY "admin_read_audit" ON lgu_audit FOR SELECT USING (auth.role() = 'authenticated');
